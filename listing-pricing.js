...
async function loadListing(container) {
    ...
    try {
        ...
        // Preview of the real eBay variation name for any queued row with
        ...
        if (needsPreview.length) {
            const { data: previews } = await supabase.rpc('render_variation_names', {
                ...
                p_variant_ids: needsPreview.map(r => r.variant_id),
            });
            const previewByVariantId = Object.fromEntries((previews || []).map(p => [p.variant_id, p.rendered_name]));
...
async function openManualMetadataModal(container, body, isDraft) {
    ...
    root.innerHTML = `
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; z-index:100; overflow-y:auto;">
            <div style="background:var(--bg-secondary); border:1px solid var(--border); border-radius:8px; padding:20px; width:480px; max-width:90vw; max-height:85vh; overflow-y:auto;">
                ...
                <textarea id="lp-meta-description-raw" rows="10" style="width:100%; font-family:monospace; font-size:12px; margin-bottom:10px; display:none;"></textarea>
    ...
    root.querySelector('#lp-desc-preview-btn').addEventListener('click', async () => {
        ...
        try {
            const result = await callDescriptionPreview(currentDescriptionHtml());
    ...
    root.querySelector('#lp-meta-save').addEventListener('click', async () => {
        ...
        const fields = {
            ...
            description_html: currentDescriptionHtml().trim() || null,
...
function openNewProfileModal(container, body, groupId) {
    ...
    root.innerHTML = `
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; z-index:100;">
            <div style="background:var(--bg-secondary); border:1px solid var(--border); border-radius:8px; padding:20px; width:460px; max-width:90vw; max-height:85vh; overflow-y:auto;">
                ...
                <label style="font-size:12px; color:var(--text-secondary); display:block; margin-bottom:10px;">Default quantity limit (optional)
                    <input type="number" id="lp-new-profile-qty-limit" placeholder="falls back to the listing's default, then 24" style="width:100%; margin-top:4px;" />
                </label>
    ...
    root.querySelector('#lp-new-profile-create').addEventListener('click', async () => {
        ...
        const qtyLimitRaw = root.querySelector('#lp-new-profile-qty-limit').value;
        const { error: profileErr } = await supabase.from('pricing_profiles').insert({
            ...
            default_quantity_limit: qtyLimitRaw ? parseInt(qtyLimitRaw, 10) : null,
        });
        if (profileErr) {
